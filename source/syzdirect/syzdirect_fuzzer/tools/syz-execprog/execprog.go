...
func (ctx *Context) printCallResults(info *ipc.ProgInfo) {
	calls := append(info.Calls, info.Extra)
	for i, inf := range calls {
		if i == len(info.Calls) {
			log.Logf(0, "Extra Call: dist: %v, signal %v, coverage %v errno %v",
				inf.Dist, len(inf.Signal), len(inf.Cover), inf.Errno)
		}
		...
		if i < len(info.Calls) {
			log.Logf(0, "CALL %v: dist: %v, signal %v, coverage %v errno %v%v",
				i, inf.Dist, len(inf.Signal), len(inf.Cover), inf.Errno, flags)
		}
	}
	log.Logf(0, "mindist: %v, hitCount: %v\n", info.MinDist, info.ProgHitCounts)
}
