...
var (
	...
	constraintInp = flag.String("inp", "constinp.json", "constraint input file")
)
...
func processJob(job *Job, descriptions *ast.Description, constFile *compiler.ConstFile) {
	...
	var prog *compiler.Prog
	if job.Target.OS == "linux" && job.Target.Arch == "amd64" && osutil.IsExist(*constraintInp) {
		log.Println("start call refining!")
		prog = compiler.CompileInGo(descriptions, consts, job.Target, eh, *constraintInp, *outDir)
	} else {
