...
var (
	...
	flagUptime   = flag.Int("uptime", 0, "fuzzing uptime (hours)")
	flagCallFile = flag.String("callfile", "", "target / relate syscall")
)
...
func main() {
	...
	if *flagUptime != 0 {
		go func() {
			timer := time.NewTimer(time.Duration(*flagUptime) * time.Hour)
			<-timer.C
			fmt.Println("Timeout!!!!")
			close(vm.Shutdown)
		}()
...
func RunManager(cfg *mgrconfig.Config) {
	...
	mgr := &Manager{
		...
		callPairMap:      prog.CallPairFromFile(*flagCallFile, cfg.Target),
	}
	...
	benchFile := filepath.Join(cfg.Workdir, "benchfile.txt")
	f, err := os.OpenFile(benchFile, os.O_WRONLY|os.O_CREATE|os.O_EXCL, osutil.DefaultFilePerm)
	if err != nil {
...
func (mgr *Manager) loadCorpus() map[int]map[int]bool {
	...
	broken := 0
	rawTcalls := mgr.callPairMap.GetRawTargetCalls(mgr.target)
	for key, rec := range mgr.corpusDB.Records {
		if !mgr.loadProg(rec.Val, minimized, smashed, rawTcalls) {
...
func (mgr *Manager) loadProg(data []byte, minimized, smashed bool, rawTcalls map[int]bool) bool {
	...
	p, _ := mgr.target.Deserialize(data, prog.NonStrict)
	hasTarget := false
...
func (mgr *Manager) saveCrash(crash *Crash) bool {
	...
	filename := filepath.Join(dir, "crashtime0")
	if !osutil.IsExist(filename) {
		osutil.WriteFile(filename, []byte(strconv.FormatInt(int64(crash.crashTime), 10)))
	}
	if mgr.cfg.BugDesc != "" && mgr.cfg.BugDesc == crash.Title {
		log.Fatalf("find the expected bug %v, %v", mgr.cfg.BugDesc, crash.Title)
	}
...
func (mgr *Manager) getFuzzingTime() time.Duration {
	return mgr.fuzzingTime
}
