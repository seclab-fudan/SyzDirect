...
func isSupported(c *prog.Syscall, target *prog.Target, sandbox string) (bool, string) {
	oriName := strings.TrimSuffix(c.Name, "$tmp_rf1")
	...
	if strings.HasPrefix(oriName, "socket$") ||
		strings.HasPrefix(oriName, "socketpair$") {
		return isSupportedSocket(c)
	}
	if strings.HasPrefix(oriName, "openat$") {
		return isSupportedOpenAt(c)
	}
	if strings.HasPrefix(oriName, "mount$") {
