...
func getCallIndexByPtr(call *Call, calls []*Call) int {
	for i, tmp := range calls {
		if tmp == call {
			return i
		}
	}
...
func (p *Prog) Clone() *Prog {
	...
	newcalls := cloneCalls(p.Calls, newargs)
	tcallIdx := getCallIndexByPtr(p.Tcall, p.Calls)
	rcallIdx := getCallIndexByPtr(p.Rcall, p.Calls)
	var rcall, tcall *Call
	if tcallIdx != -1 {
		tcall = newcalls[tcallIdx]
	}
	if rcallIdx != -1 {
		rcall = newcalls[rcallIdx]
	}
	p1 := &Prog{
		Target: p.Target,
		Calls:  newcalls,
