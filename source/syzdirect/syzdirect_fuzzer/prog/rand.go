...
func (a *ResourceType) generate(r *randGen, s *state, dir Dir) (arg Arg, calls []*Call) {
	if r.rcall != nil && dir != DirOut {
		var resources []*ResultArg
		ForeachArg(r.rcall, func(arg Arg, _ *ArgCtx) {
			// Collect only initialized resources (the ones that are already used in other calls).
			a0, ok := arg.(*ResultArg)
			if !ok || a0.Dir() != DirOut ||
				!r.target.isCompatibleResourcePrecise(a.TypeName, a0.Type().Name()) {
				return
			}
			resources = append(resources, a0)
		})
		if len(resources) > 0 {
			resource := resources[r.Intn(len(resources))]
			newArg := MakeResultArg(a, dir, resource, 0)
			r.rcall = nil
			return newArg, nil
		}
	}
