...
func (p *Prog) Mutate(rs rand.Source, ncalls int, ct *ChoiceTable, corpus []*Prog) {
	...
	for stop, ok := false, false; !stop; stop = ok && len(p.Calls) != 0 && r.oneOf(3) {
		...
		case r.nOutOf(1, 100):
			ok = ctx.splice()
		case r.nOutOf(20, 31):
			x := float64(time.Since(ct.startTime) / time.Minute)
	...
	if p.Tcall == nil {
		p.Target.FixExtraCalls(p, r.Rand, ct, RecommendedCalls, nil)
	}
	p.sanitizeFix()
	p.debugValidate()
...
func (ctx *mutator) insertCall() bool {
	...
	// idx := int(r.rand(getCallIndexByPtr(ctx.p.Tcall, ctx.p.Calls) + 2))
	var c *Call
	if idx < len(p.Calls) {
		c = p.Calls[idx]
	}
	...
	for len(p.Calls) > ctx.ncalls {
		isSucc := p.RemoveCall(idx)
		if !isSucc {
			log.Fatalln("there should not fail")
		}
	}
	return true
}
...
func (ctx *mutator) mutateArg(idx int) bool {
	p, r := ctx.p, ctx.r
	if len(p.Calls) == 0 {
		return false
	}
...
func chooseCall(p *Prog, r *randGen) int {
	...
	for _, c := range p.Calls {
		...
		if c == p.Tcall || c == p.Rcall {
			totalPrio = totalPrio * 0.2
		}
		prioSum += totalPrio
