...
func removeCalls(p0 *Prog, callIndex0 int, crash bool, pred func(*Prog, int) bool) (*Prog, int) {
	for i := len(p0.Calls) - 1; i >= 0; i-- {
		...
		p := p0.Clone()
		if p.Calls[i] == p.Tcall {
			p.Tcall = nil
			p.Rcall = nil
		} else if p.Calls[i] == p.Rcall {
			p.Rcall = nil
		}
		p.RemoveCall(i)
		if !pred(p, callIndex) {
...
/// if tcall is remove, return true
func (p *Prog) MiniRemoveCall(idx int) (ret bool) {
	ret = false
	if p.Calls[idx] == p.Tcall {
		p.Tcall = nil
		ret = true
	} else if p.Calls[idx] == p.Rcall {
		p.Rcall = nil
	}
	p.RemoveCall(idx)
	return
