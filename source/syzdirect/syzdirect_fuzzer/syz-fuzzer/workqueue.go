...
func (wq *WorkQueue) dequeue() (item interface{}) {
	wq.mu.RLock()
	if len(wq.triageCandidate)+len(wq.candidate)+wq.triage.Size()+wq.smash.Size() == 0 {
		wq.mu.RUnlock()
		return nil
	}
	wq.mu.RUnlock()
...
func (wq *WorkQueue) getSize() (int, int, int) {
	wq.mu.RLock()
	defer wq.mu.RUnlock()
	return wq.triage.Size(), wq.smash.Size(), len(wq.candidate)
}
