...
static const int DIST_SIZE = 300;
...
void write_call_output(thread_t* th, bool finished)
{
	...
	uint32* hit_num_pos = write_output(0); 
	...
	if (flag_comparisons) {
		...
		*dist_pos = th->cov.dist_area[0];
		if (is_kernel_64_bit)
			write_coverage_signal<uint64>(&th->cov, signal_count_pos, cover_count_pos, hit_num_pos);
		else
			write_coverage_signal<uint32>(&th->cov, signal_count_pos, cover_count_pos, hit_num_pos);
	}
...
void write_extra_output()
{
	...
	uint32* hit_num_pos = write_output(0);
	if (is_kernel_64_bit)
		write_coverage_signal<uint64>(&extra_cov, signal_count_pos, cover_count_pos, hit_num_pos);
	else
		write_coverage_signal<uint32>(&extra_cov, signal_count_pos, cover_count_pos, hit_num_pos);
	cover_reset(&extra_cov);
