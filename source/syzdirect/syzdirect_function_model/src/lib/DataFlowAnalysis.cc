...
void DataFlowAnalysis::findUseswithTrackLoad(Instruction *BorderInsn, Value *Target,
                                             set<use_t> &UseSet, set<Value *> &Visited, bool trackLoad) {
    ...
    for (User *U : Target->users()) {
        ...
        if (GetElementPtrInst *GEP = dyn_cast<GetElementPtrInst>(U)) {
            if (reachBBs.find(BB) != reachBBs.end()) {
                ...
                if (GEP->getPointerOperand() == Target) {
                    findUseswithTrackLoad(BorderInsn, U, UseSet, Visited,trackLoad);
                }
            }
            continue;
        }
            ...
            if (Target == SI->getPointerOperand()) {
                ...
                for (Value *A : AliasSet) {
                    for (User *AU : A->users()) {
                        ...
                        if (LI)
                            findUseswithTrackLoad(BorderInsn, AU, UseSet, Visited,trackLoad);
                    }
                }
            }
            continue;
        ...
        if(SelectInst* SI =  dyn_cast<SelectInst>(U)){
            findUseswithTrackLoad(BorderInsn, SI, UseSet, Visited,trackLoad);
            continue;
        }
        ...
        if (PHINode *PHI = dyn_cast<PHINode>(U)) {
            findUseswithTrackLoad(BorderInsn, PHI, UseSet, Visited,trackLoad);
            continue;
        }
...
void DataFlowAnalysis::findUseswithTrackLoad(Instruction *BorderInsn, Value *Target, set<Value*> &aliasSet, 
												set<Value*> &childrenSet, map<PHINode*, Value*> &phiValueMap,
                                             	set<use_t> &UseSet, set<Value *> &Visited, bool trackLoad) {
    ...
    for (User *U : Target->users()) {
        ...
        if (GetElementPtrInst *GEP = dyn_cast<GetElementPtrInst>(U)) {
            if (reachBBs.find(BB) != reachBBs.end()) {
                UseSet.insert(use_c(Insn, -1));
                if (GEP->getPointerOperand() == Target) {
					unsigned int OpNum = GEP->getNumOperands();
					// TODO: check whether get a positive offset
					if (aliasSet.count(Target)) {
						childrenSet.insert(V);
					} else if (childrenSet.count(Target)) {
						childrenSet.insert(V);
					}
                    findUseswithTrackLoad(BorderInsn, U, aliasSet, childrenSet, phiValueMap, UseSet, Visited, trackLoad);
                }
            }
            continue;
        }
            ...
            if (Target == SI->getPointerOperand()) {
                ...
                for (Value *A : AliasSet) {
                    for (User *AU : A->users()) {
                        ...
                        LoadInst *LI = dyn_cast<LoadInst>(AU);
                        if (LI)
                            findUseswithTrackLoad(BorderInsn, AU, aliasSet, childrenSet, phiValueMap, UseSet, Visited,trackLoad);
                    }
                }
            }
            continue;
        ...
        if(SelectInst* SI =  dyn_cast<SelectInst>(U)){
            findUseswithTrackLoad(BorderInsn, SI, aliasSet, childrenSet, phiValueMap, UseSet, Visited,trackLoad);
            continue;
        }
        ...
        if (PHINode *PHI = dyn_cast<PHINode>(U)) {
			if (phiValueMap.count(PHI)) {
				if (aliasSet.count(phiValueMap[PHI])) {
					aliasSet.insert(V);
				} else if (childrenSet.count(phiValueMap[PHI])) {
					childrenSet.insert(V);
				}
			}
            findUseswithTrackLoad(BorderInsn, PHI, aliasSet, childrenSet, phiValueMap, UseSet, Visited,trackLoad);
            continue;
        }
        //Use 3: Taking parameters in a function call
        if (CallInst *CI = dyn_cast<CallInst>(U)) {
...
void DataFlowAnalysis::findInFuncSourceCV(Value *V, set<Value *> &SourceSet, 
		set<Value *> &CVSet, set<Value *> &TrackedSet, bool traceAlias) {
	...
	if (CallInst *CI = dyn_cast<CallInst>(V)) {
        ...
        if (CF->getName().contains("with.overflow")) {
            for (unsigned int i = 0; i < CI->getNumArgOperands(); ++i)
                findInFuncSourceCV(CF->getOperand(i), SourceSet, CVSet, TrackedSet);
            return;
        }else{
            SourceSet.insert(CI);
            return;
	...
	if (CastInst *BI = dyn_cast<CastInst>(V)) {
		CVSet.insert(BI);
		findInFuncSourceCV(BI->getOperand(0), SourceSet, CVSet, TrackedSet);
		return;
	}

    if (ReturnInst *RI = dyn_cast<ReturnInst>(V)) {
        CVSet.insert(RI);
        findInFuncSourceCV(RI->getOperand(0), SourceSet, CVSet, TrackedSet);
        return;
    }
	SelectInst *SI = dyn_cast<SelectInst>(V);
	if (SI) {
...
//Collect simple uses for a Target using forward dataflow analysis,a Simplified version of findUses
//it has no memory analysis and the propagation is limited
void DataFlowAnalysis::findSimpleUses( llvm::Instruction * BorderInsn,  llvm::Value * Target, set<use_t> & UseSet, set< llvm::Value *> & Visited) {
    if (!BorderInsn)
        return;
    ...
    for (User *U : Target->users()) {
        ...
        if (PHINode *PHI = dyn_cast<PHINode>(U)){
            findSimpleUses(BorderInsn, PHI, UseSet, Visited);
            continue;
        }
        if(BitCastInst* BC = dyn_cast<BitCastInst>(U)){
            findSimpleUses(BorderInsn, BC, UseSet, Visited);
            continue;
        }
        if (CallInst *CI = dyn_cast<CallInst>(U)) {
            if (reachBBs.find(BB) != reachBBs.end()) {
...
void DataFlowAnalysis::findUses(Instruction *BorderInsn, Value *Target, 
	...
	for (User *U : Target->users()) {
		...
		LoadInst *LI = dyn_cast<LoadInst>(U);
        if (LI) {
		    if (Target == LI->getPointerOperand()){
                // Used as the address of a load operation
                // Mark it as a critical use
                if (reachBBs.find(BB) != reachBBs.end()) {
                    UseSet.insert(use_c(Insn, -1));
        ...
        if (PHINode *PHI = dyn_cast<PHINode>(U)){
            findUses(BorderInsn, PHI, UseSet, Visited);
            continue;
        }
		//Use 3: Taking parameters in a function call
		if (CallInst *CI = dyn_cast<CallInst>(U)) {
...
bool DataFlowAnalysis::isNotStored(Value *V) {
