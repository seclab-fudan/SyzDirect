...
// #define SOUND_MODE 1
#define MAX_BLOCKS_SUPPORT 1000

//
// Function modeling
//

static void SetAllocFuncs(set<string> &AllocFuncs) {
	string exepath = sys::fs::getMainExecutable(NULL, NULL);
	string exedir = exepath.substr(0, exepath.find_last_of('/'));
	string line;
	ifstream errfile(exedir + "/config/alloc-funcs");
	if (errfile.is_open()) {
		while (!errfile.eof()) {
			getline(errfile, line);
			if (line.length() > 1) {
				AllocFuncs.insert(line);
			}
		}
		errfile.close();
	}
...
static void SetGroundTruthFuncs(set<string> &GroundTruthFuncs) {

        string exepath = sys::fs::getMainExecutable(NULL, NULL);
        string exedir = exepath.substr(0, exepath.find_last_of('/'));
        string line;
        ifstream errfile(exedir + "/configs/gt-funcs");
        if (errfile.is_open()) {
                while (!errfile.eof()) {
                        getline (errfile, line);
                        if (line.length() > 1) {
                                GroundTruthFuncs.insert(line);
                        }
                }
                errfile.close();
        }
