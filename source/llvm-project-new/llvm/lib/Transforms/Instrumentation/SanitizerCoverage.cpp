...
void ModuleSanitizerCoverage::loadDistanceInfo() {
    ...
    if(sys::fs::is_regular_file(filePath) && filePath.endswith(".dist")) {
      ...
      if (distanceFile.is_open()) {
        while (!distanceFile.eof()) {
          ...
          // errs() << "debug1: " << relatePathSig << "|" << funcName << "|" << blockIdx << "|" << blockDistance << "\n";
  ...
  // errs() << "faaa: " << BlockDistanceInfo.size() << "\n";
  // for (auto item: BlockDistanceInfo) {
  //   errs() << "faa: " << item.first << "\n";
  //   for (auto subitem: item.second) {
  //     errs() << "fbb: " << subitem.first << "\n";
  //   }
  // }
  // errs() << "===========\n";
}
...
bool ModuleSanitizerCoverage::instrumentModule(
  ...
  // errs() << "debug1: " << Options.CompileTime << "\n";
  // errs() << "debug1: " << Options.KernelSrcDir << "\n";
  // errs() << "debug1: " << Options.DistanceDir << "\n";
  // errs() << "debug1: " << Options.TargetFunction << "\n";
  StringRef relatePathSig;
  std::map<std::string, std::map<unsigned int, unsigned int>> blockDistanceInfoModule;
  if (Options.CompileTime == 2) {
    StringRef moduleSrcPath = StringRef(M.getSourceFileName());// /a/b/c/e/f/g.c
    StringRef kernelSrcPath = StringRef(Options.KernelSrcDir); // /a/b/c/ or /a/b/c
    if (moduleSrcPath.startswith(kernelSrcPath)) {
      if (kernelSrcPath.endswith("/")) {
    ...
    if(resStr.first==relatePathSig && resStr.second==""){
      errs() << "moudle path error! (no dot found in path)" << "\n";
      errs() << moduleSrcPath << "\n";
      errs() << kernelSrcPath << "\n";
      exit(-1);
    }
    ...
    std::string relatePathSigStr(tmpStr); // e-f-g
    // errs() << "relate path: " << relatePathSig << "\n";
    blockDistanceInfoModule = BlockDistanceInfo[relatePathSigStr];
    // errs() << BlockDistanceInfo.size() << "\n";
    // for (auto item: BlockDistanceInfo) {
    //   errs() << item.first << "\n";
    // }
    // errs() << blockDistanceInfoModule.size() << "\n";
    // errs() << blockDistanceInfoModule["main"].size() << "\n";
  }
  ...
  if (Options.CompileTime == 2)
    SanCovTracePC = M.getOrInsertFunction(SanCovTracePCName, Int32Ty);
  else
...
void ModuleSanitizerCoverage::InjectCoverageAtBlock(Function &F, BasicBlock &BB,
  ...
  if (Options.TracePC) {
    if (Options.CompileTime == 2) {
      IRB.CreateCall(SanCovTracePC, {IRB.getInt32(distance)})
